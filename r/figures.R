...
biomass <- read.csv('res_atpm/obj_wt_sty.csv', header = FALSE)
phb <- read.csv('res_atpm/phb_wt_sty.csv', header = FALSE)

wt <- cbind(biomass, phb)
colnames(wt) <- c('biomass', 'phb_lb', 'phb_ub')
wt['color'] <- 'wt'

biomass <- read.csv('res_atpm/obj_pta_sty.csv', header = FALSE)
phb <- read.csv('res_atpm/phb_pta_sty.csv', header = FALSE)

pta <- cbind(biomass, phb)
colnames(pta) <- c('biomass', 'phb_lb', 'phb_ub')
pta['color'] <- 'pta'

biomass <- read.csv('res_atpm/obj_ack_sty.csv', header = FALSE)
phb <- read.csv('res_atpm/phb_ack_sty.csv', header = FALSE)

ack <- cbind(biomass, phb)
colnames(ack) <- c('biomass', 'phb_lb', 'phb_ub')
ack['color'] <- 'ack'

biomass <- read.csv('res_atpm/obj_ex_sty.csv', header = FALSE)
phb <- read.csv('res_atpm/phb_ex_sty.csv', header = FALSE)
...
pe_wt +
    ...
    geom_ribbon(data = subset(ex), aes(x = biomass, ymin = phb_lb, ymax = phb_ub), fill = 'darkorchid', alpha = 0.1) +
    ...
    guides(color = guide_legend(override.aes = list(size = .5))) +
    theme_minimal() +
    ggtitle('Coupling PHB Production to ATPM (Styrene M9)') +
    # xlab(expression ('Growth Rate '~(h^-1))) +
    xlab('ATP Synthesis (mmol/gDWh)') +
    ylab('PHB Production (mmol/gDWh)') +
    scale_x_continuous(expand = c(0,0)) +
...
ggsave(filename = "atpm_ac_tca_sty_envelope.png", path = "figures/", width = 6, height = 5, device='png', dpi=1400)
